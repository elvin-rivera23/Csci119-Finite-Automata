...
nap ts q a = norm [ q' | (q1, c, q') <- ts, q == q1, a == c]
...
naccept m w = or [overlap (ndelta_star m q w) (nfinals m)| q <- (nstarts m) ]
...
eclose m qs = norm $ stable $ iterate close (qs, []) where
...
edelta_star m q (a:w) = norm $ eclose m (norm [x'' | x <- eclose m [q], x' <- eclose m (nap (edelta m) x a), x'' <- edelta_star m x' w])
...
nfsm_to_fsm m = FSM {
	...
	start = nstarts m,
	finals = [ f | f <- subsequences (nstates m), overlap f (nfinals m)],
...
efsm_to_fsm m = FSM {
	...
	finals = [f | f <- subsequences (estates m), overlap f (efinals m)],
